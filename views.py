...
def detail(request, recipe_id, error_message=None):
    ...
    return HttpResponse(loader.get_template('cookbook/detail.html').render(context, request))
...
def advanced_recipe_search_results(request):
    ...
    if food_groups:
        for food_group_id in food_groups:
            SearchFoodGroup(search=saved_search, food_group=FoodGroup.objects.get(pk=food_group_id), include=True).save()
            if tags:
                for tag_id in tags:
                    SearchTag(search=saved_search, tag=Tag.objects.get(pk=tag_id), include=True).save()
...
def saved_search_detail(request):
    return render(request, 'cookbook/saved_search_detail.html')
