...
def execute_saved_search(saved_search):
    ...
    if saved_search.recipe_search_term:
        results = results.filter(
            title__icontains=saved_search.recipe_search_term)
    if saved_search.ingredient_search_term:
        results = results.select_related().filter(
            recipeingredient__ingredient__in=Ingredient.objects.filter(
                name__icontains=saved_search.ingredient_search_term))
    if saved_search.searchtag_set.all().exists():
        results = results.filter(
            recipetag__tag__in=Tag.objects.select_related().filter(
                searchtag__search=saved_search))
    if saved_search.searchfoodgroup_set.all().exists():
        results = results.exclude(~Q(
            recipeingredient__ingredient__in=Ingredient.objects.filter(
                food_group__in=FoodGroup.objects.select_related().filter(
                    searchfoodgroup__search=saved_search))))
